...
class NetworkService {
    ...
    func request(searchTerm: String, latitude: Float? = nil, longitude: Float? = nil, completion: @escaping (Data?, Error?) -> Void)  {
        
        let parameters = self.prepareParaments(searchTerm: searchTerm, latitude: latitude, longitude: longitude, radius: 5)
        let url = self.url(params: parameters)
        var request = URLRequest(url: url)
        request.httpMethod = "get"
        let task = createDataTask(from: request, completion: completion)
    ...
    private func prepareParaments(searchTerm: String?, latitude: Float? = nil, longitude: Float? = nil, radius: Int? = nil) -> [String: String] {
